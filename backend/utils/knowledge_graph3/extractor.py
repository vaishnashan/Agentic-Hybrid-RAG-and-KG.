...
then loads them into Neo4j (Aura) after validating each one against schema.py.
...
set GROQ_MODEL=openai/gpt-oss-120b in .env for higher quality at more cost/latency.
Note: Groq deprecated llama-3.1-8b-instant / llama-3.3-70b-versatial in June 2026 —
...
from codebase.backend.utils.ingestion1.loader import PROCESSED_DIR
from codebase.backend.utils.knowledge_graph3.neo4j_client import Neo4jClient
from codebase.backend.utils.knowledge_graph3.schema import (
    CONCEPT_KEYWORDS,
    NODE_LABELS,
    RELATION_SHAPES,
    SEED_CONCEPTS,
...
# in _validate_and_normalize_triples before anything reaches Neo4j.
...
def _validate_and_normalize_triples(raw_triples: List[dict], arxiv_id: str) -> List[dict]:
    ...
    to Neo4j" — nothing skips this check.
    """
    valid: List[dict] = []
