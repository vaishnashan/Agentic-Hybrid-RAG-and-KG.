"""
Combines the dense (Chroma) and sparse (BM25) result lists into one ranked
list, using Reciprocal Rank Fusion (RRF).

Why RRF instead of just averaging the two scores: dense similarity scores
(roughly 0-1) and BM25 scores (unbounded, can be 0-20+) are on completely
different scales, so directly comparing or averaging them is meaningless.
RRF sidesteps this by only looking at each chunk's RANK (position) in each
list, not its raw score — a chunk ranked #1 in both lists beats a chunk
ranked #1 in only one list, regardless of the underlying score scales.
...
def reciprocal_rank_fusion(result_lists: List[List[RetrievedChunk]], k: int = RRF_K) -> List[RetrievedChunk]:
    ...
    for chunk_id, fused_score in fused_ranking:
        ...
        merged.append(
            RetrievedChunk(
                chunk_id=base.chunk_id, text=base.text, score=fused_score,
                metadata=base.metadata, source="hybrid",
            )
        )
    return merged
