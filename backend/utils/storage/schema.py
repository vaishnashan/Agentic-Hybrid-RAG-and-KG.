"""
Knowledge graph schema for the paper corpus.

This is the single source of truth for what a "valid triple" is allowed to
look like. graph_index.py checks every LLM-produced triple against
RELATION_SHAPES before it's written to Neo4j — anything that doesn't match a
listed (subject_type, relation, object_type) combination is dropped rather
than stored, so the graph can't silently fill up with relation types nobody
agreed on (e.g. "USES" vs "USES_TECHNIQUE" vs "APPLIES" all meaning the same
thing).
...
Relationship types (see RELATION_SHAPES below for exactly which node-type
pairs each relation is allowed to connect):
...
GRAPH BACKEND: Neo4j Aura (managed cloud, free tier). Unlike a self-hosted
Redis-module graph DB, Aura DOES support real `IS UNIQUE` constraints — see
CONSTRAINTS_CYPHER below, which neo4j_client.py actually runs (not just kept
for reference).
"""

NODE_LABELS = ["Paper", "Concept", "Method", "Dataset", "Task"]

# Which property uniquely identifies a node of each label. Paper is keyed by
# its arxiv_id (stable external ID); everything else is keyed by its literal
# name, since there's no better natural key for a concept/method/dataset/task.
NODE_KEY_PROPERTY = {
    "Paper": "arxiv_id",
    "Concept": "name",
    "Method": "name",
...
# The allowed (subject_type, object_type) pairs for each relation. A triple
# is only valid if its relation is a key here AND its (subject_type,
# object_type) appears in that relation's list. This is what stops the
# LLM's freeform output from turning into an unbounded set of near-duplicate
# relation types.
RELATION_SHAPES = {
...
# Real uniqueness constraints — Neo4j Aura enforces these at the database
# level (unlike a self-hosted Redis-module graph DB), so a duplicate node
# can never be created even by a buggy MERGE. neo4j_client.py runs these on
# startup.
CONSTRAINTS_CYPHER = [
...
# Seed vocabulary — passed to the LLM as PREFERRED concept names to reuse
# when a match genuinely fits, so the graph doesn't fragment into
# near-duplicate concept nodes ("self-evolution" vs "self-evolving skills").
...
# Free, no-API-key fallback if the LLM call fails for a given paper (see
# graph_index.py's _extract_with_keywords).
CONCEPT_KEYWORDS = {
...
def is_valid_triple(subject_type: str, relation: str, object_type: str) -> bool:
    """The single validation check graph_index.py runs on every LLM-produced triple."""
