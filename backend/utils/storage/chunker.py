"""
Chunks the already-extracted papers from data/processed/papers.jsonl
(produced by loader.py) — does NOT touch the PDFs or re-run extraction, so
this can be run independently and repeatedly while tuning chunk size/
overlap, without waiting for extraction to re-run.

LangChain's RecursiveCharacterTextSplitter does the actual text splitting.
Custom heading detection is used only to (a) tag each chunk with its section
name and (b) exclude the References section before any text reaches the
splitter.
...
from .config import CHARS_PER_WORD, CHUNK_OVERLAP_WORDS, CHUNK_SIZE_WORDS, CHUNKS_PATH, PAPERS_PATH
from .loader import PaperRecord
...
def load_processed_papers(path: Path = PAPERS_PATH) -> List[PaperRecord]:
...
def chunk_all(papers: List[PaperRecord]) -> List[Chunk]:
    """Chunks every paper, printing per-paper progress."""
    print("=" * 70)
    ...
    for index, paper in enumerate(papers, start=1):
        ...
        print(
            ...
            f"({paper.category}) -> {len(paper_chunks)} chunks ({elapsed:.2f}s)"
        )
...
def run_chunker() -> List[Chunk]:
    """Runs the complete chunking step. Used by pipeline.py."""
    papers = load_processed_papers()
    print(f"Loaded {len(papers)} processed papers from {PAPERS_PATH}\n")
