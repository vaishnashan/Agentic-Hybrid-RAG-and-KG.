"""
Builds the knowledge graph: reads data/processed/papers.jsonl, and for each paper
asks an LLM (Groq) to extract schema-controlled (subject, relation, object) triples,
then loads them into FalkorDB (self-hosted, free) after validating each one against
schema.py.

Why triples instead of two flat lists (old approach): a flat "concepts" +
"methods" list can only ever express (Paper)-[:MENTIONS_CONCEPT]->(Concept) and
(Paper)-[:USES_METHOD]->(Method). It can't say "this method PROPOSES that
method" or "this paper EVALUATED_ON this dataset" or "this method SUPPORTS this
concept" — richer relationships that make the graph useful for real multi-hop
questions. Asking the LLM for triples directly lets it express any of the
relation shapes defined in schema.py, in one call.
...
Every triple the LLM returns is checked against schema.RELATION_SHAPES before
it's written; anything that doesn't match a known (subject_type, relation,
object_type) shape is dropped (and logged), never silently stored.

Falls back to the old keyword-matching approach (schema.CONCEPT_KEYWORDS) for any
paper where the LLM call fails or returns no valid triples — so a flaky API call
never crashes the whole build, it just degrades that one paper's extraction to
Paper-MENTIONS_CONCEPT-Concept triples only.

Requires GROQ_API_KEY in your .env. Uses openai/gpt-oss-20b by default (fast, cheap,
Groq's recommended low-cost default for general text extraction); set
GROQ_MODEL=llama-3.3-70b-versatile in .env if you want to A/B test extraction
quality against a larger model — compare the "fell back to keyword extraction"
count this script prints at the end to judge whether the upgrade is worth the
extra cost. Note: Groq deprecated some older Llama model names in June 2026 —
if you see model-not-found errors, check https://console.groq.com/docs/models for
the current model list.

GRAPH BACKEND: writes to FalkorDB Cloud (free tier, no credit card required)
instead of Neo4j Aura — see falkordb_client.py for connection details.
"""
import json
import os
import re
...
from codebase.backend.utils.storage.knowledge_graph3.falkordb_client import FalkorGraphClient
...
# How much of full_text gets sent to the LLM per paper. 2500 chars was
# abstract-only; this is widened to also catch the intro/methods section, where
# dataset names and task framing usually show up (abstracts rarely mention them).
...
# Literal placeholder the LLM uses instead of guessing the paper's arxiv_id (it
# only sees the title/text, never the ID) — swapped back to the real arxiv_id
# in _validate_and_normalize_triples before anything reaches the graph DB.
...
def load_papers(path: Path = PAPERS_PATH) -> List[dict]:
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"(python -m utils.ingestion.loader)."
        )
    papers = []
...
def _validate_and_normalize_triples(raw_triples: List[dict], arxiv_id: str) -> List[dict]:
    ...
    THIS_PAPER placeholder for the paper's real arxiv_id. This is the one place
    that stands between "whatever the LLM said" and "what actually gets written
    to the graph DB" — nothing skips this check.
    """
    valid: List[dict] = []
...
def build_knowledge_graph() -> Dict[str, int]:
    ...
    print(f"Extraction mode: {'LLM (Groq, ' + GROQ_MODEL + ')' if using_llm else 'keyword fallback only (no GROQ_API_KEY set)'}\n")
    ...
    for index, paper in enumerate(papers, start=1):
        ...
        # Track this paper's concepts for the co-occurrence pass below.
