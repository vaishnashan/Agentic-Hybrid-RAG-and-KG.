...
before it's written to the graph DB — anything that doesn't match a listed
...
add the label here, an index below, and a relation shape for it.

GRAPH BACKEND NOTE: this project runs on FalkorDB (self-hosted, free, Redis-module
based) instead of Neo4j Aura. FalkorDB speaks a subset of openCypher, so almost
all Cypher below is unchanged — the one real difference is that FalkorDB (unlike
Neo4j Enterprise/Aura) does not enforce `IS UNIQUE` constraints the same way, so
uniqueness is enforced purely by our own MERGE-on-key-property pattern in
falkordb_client.py, not by the database. FALKORDB_INDEX_CYPHER below just adds
range indexes for query speed — it is NOT a uniqueness guarantee.
"""
...
# Kept only for reference / if you ever migrate back to Neo4j — NOT used by
# falkordb_client.py, since FalkorDB doesn't support `IS UNIQUE` constraints
# the way Neo4j Enterprise/Aura does.
CONSTRAINTS_CYPHER = [
...
# What falkordb_client.py actually runs. FalkorDB supports the simpler
# `CREATE INDEX FOR (n:Label) ON (n.prop)` form (no "IF NOT EXISTS", no
# uniqueness). These just speed up MATCH/MERGE lookups on the key property —
# uniqueness itself is enforced in code (falkordb_client.py's MERGE-by-key
# pattern), not by the database.
FALKORDB_INDEX_CYPHER = [
    ...
    "CREATE INDEX FOR (t:Task) ON (t.name)",
]
